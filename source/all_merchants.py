...
# Primary ranking metric for portfolio comparison (other metrics are also saved)
chosen_metric = "MAPE"
...
def evaluate_one_merchant(df_merchant, m_id, chosen_metric=chosen_metric):
    ...
    # Full nine-model portfolio benchmark
    preds = {
...
def main():
    ...
    if all_metrics:
        ...
        leaderboard.to_excel(TRANSFORMED_DIR / "leaderboard.xlsx", index=False)

    if all_best:
        best_all = pd.concat(all_best, ignore_index=True)
        best_all.to_excel(TRANSFORMED_DIR / "best_model_by_merchant.xlsx", index=False)
